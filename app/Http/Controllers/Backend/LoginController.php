...
class LoginController extends Controller
{
    ...
    public function getChangePassword(Request $request)
    {
        ...
        return redirect('login')->with('error','Link cập nhật mật khẩu không hợp lệ hoặc đã hết hạn');
    }
    ...
    public function postChangePassword(Request $request)
    {
        ...
        if(!empty($user) && !empty($token))
        {
            ...
            $user->reset_token ='';
            if($user->save())
                return redirect('login')->with('success','Mật khẩu đã được thay đổi thành công, bạn có thể đăng nhập bằng mật khẩu mới ngay bây giờ');
        }
        return redirect('login')->with('error','Mã bảo vệ hết hạn hoặc không đúng, vui lòng thử lại!');
    }
}
